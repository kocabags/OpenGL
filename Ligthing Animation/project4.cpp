...
const char *WINDOWTITLE = { "Project 4 -- Sarp Kocabagli" };
...
Display( )
{
	...
	glPushMatrix();
	glShadeModel ( GL_FLAT );
	SetMaterial ( 1. , 0. , 0.2 , -100. );
	MjbSphere( 1., 50, 50);
	glPopMatrix();

	//White pointlight
	SetPointLight ( GL_LIGHT0, 0. , 2. *Time , 2. , 1. , 1. , 1. );
	glPushMatrix();
	if ( Light0On ) {
		glEnable ( GL_LIGHT0 );
	}
	...
	//Red Pointlight
	SetPointLight ( GL_LIGHT1, 3. , 2. , 10 , 0.3 , 1. , 0.5 );
	glPushMatrix();
	if ( Light1On ) {
		glEnable ( GL_LIGHT1 );
	}
	...
	glPushMatrix();
	glShadeModel ( GL_SMOOTH );
	SetMaterial ( 0.3 , 1. , 0.5 , 100. );
	glTranslatef ( 3. , 2. , 10 );
    MjbSphere( .1 , 50, 50);
	glPopMatrix();


	SetSpotLight ( GL_LIGHT2 , 0. , 2. , -4. , 0. , -0.3 , -1. , 2. , 2. , 0. );
	glPushMatrix();
	if ( Light2On ) {
		glEnable ( GL_LIGHT2 );
	}
	...
	glTranslatef ( -0.4 * Time , 0. , -5. );
	SetMaterial ( 0.6 , 0.7 , 0.6 , 100. );
